...
public class TankCtrl : MonoBehaviour
{
    ...
    void Attack()
    {
        ...
        if (att_Delay > 0.0f)   // 탱크 공격 딜레이
            return;

        if (GameMgr.Inst.tower_List.Count <= 0)     // 게임 내에 타워가 하나도 없을 경우
        {
            target_Obj = null;
            return;
        }
        ...
        att_Delay = attRate;                    // 공격 딜레이를 공격 속도 많큼 설정
        GameObject bullet = Instantiate(bullet_Obj, fire_Pos.transform.position, fire_Pos.transform.rotation);      // 총알 오브젝트 생성
        bullet.GetComponent<BulletCtrl>().target_Obj = target_Obj;      // 총알 오브젝트의 타겟을 설정
        bullet.GetComponent<BulletCtrl>().bullet_Damage = atk;      // 총알을 대미지를 설정
        GameObject explo_Obj = Instantiate(turret_Explo, fire_Pos.transform.position, Quaternion.identity);     // 폭발 이펙트 생성
